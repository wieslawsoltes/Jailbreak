 ...
 */
export function installEditorHistory({document:doc=globalThis.document,active,documents,notify,shared=null}){
  ...
  const input=event=>{if(shared||applying||editor.readOnly)return;const file=active(),before=documents()[file];if(typeof before!=='string')return;
  ...
  function apply(direction){if(editor.readOnly)return;try{if(shared){shared(direction);return;}const edit=history.apply(active(),editor.value,direction);if(!edit)return;
