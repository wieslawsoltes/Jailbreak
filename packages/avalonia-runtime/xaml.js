...
export function createXamlRuntime(api, types, documents) {
  ...
  function attr(target,name,value,scope){
    ...
    if(value?.kind==='resource'&&value.dynamic){scope.pending.push(()=>{
      const key=keyOf(value.key),update=()=>{const found=findResource(key,target);if(found.found)set(target,name,found.value);else target.ClearValue(name);};update();
      for(let p=target;p;p=p.parent)if(p.Resources)target.track(p.Resources.subscribe(k=>{if(k===key)update();}));target.track(applicationResources.subscribe(k=>{if(k===key)update();}));
    });return;}
  ...
  function setters(node,parent,scope){
    const result={};for(const child of node.children.flatMap(c=>c.kind==='property'&&c.property==='Setters'?c.children:[c])){
      ...
      result[name]=valueNode?build(valueNode,parent,scope):resolve(child.attributes.Value,parent,scope);
    }return result;
  }
  function object(node,parent,scope){
    ...
    if(node.type==='ControlTheme'){
      ...
      return {kind:'controlTheme',targetType,basedOn:resolve(a.BasedOn,parent,scope),setters:setters(node,parent,scope)};
    }
    if(node.type==='Style')return {selector:a.Selector??'*',setters:setters(node,parent,scope)};
