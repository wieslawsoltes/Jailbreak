...
export function linkBinaryCompilation(files,binary,options={}){
  ...
  return {...source,success,code:success?prefix+source.code:'',diagnostics,binaries:binary.assemblies,packages:binary.packages??[],manifest:{...source.manifest,binaryAssemblies:binary.assemblies.map(a=>({name:a.name,version:a.descriptor.version}))}};
}
export function compileBinaryProject(files,assemblies,options={}){return linkBinaryCompilation(files,compileAssemblies(assemblies,options),options);}
