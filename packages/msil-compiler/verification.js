...
export function verifyMethod(assembly,method,{maxInstructions=100000}={}){
  ...
  if(method.body.hasExceptionSections)error('Exception regions are not yet supported',undefined,'JB6104');
  ...
  for(let cursor=0;cursor<pending.length;cursor++){
    ...
    else if(compares.has(op)){const b=pop(),a=pop();if(a!==b||!['i4','i8','f','ref'].includes(a)||(a==='ref'&&!['ceq','beq','bne.un','cgt.un'].includes(op)))error('Invalid comparison operands',i);types.set(offset,a);if(op.startsWith('b')){target(i.operand,stack);branch=true;}else push('i4');}
    else if(op==='br'){target(i.operand,stack);terminal=true;}
    else if(op==='brfalse'||op==='brtrue'){const type=pop();if(!['i4','i8','ref'].includes(type))error('Invalid branch condition',i);target(i.operand,stack);branch=true;}
    else if(op==='switch'){pop('i4');for(const to of i.operand)target(to,stack);branch=true;}
    ...
    else if(op==='ret'){const ret=stackType(method.signature.returnType);if(ret!=='void')pop(ret);if(stack.length)error('Nonempty stack at return',i);terminal=true;}
    else if(op==='throw'){pop('ref');terminal=true;}
    else if(!['nop','break'].includes(op))error('Unsupported IL opcode '+i.name,i,'JB6104');
    if(!terminal)target(i.next,stack);
  }
  ...
  return {instructions,states,types,calls:[...calls],fields:[...fieldRefs],peak};
}
