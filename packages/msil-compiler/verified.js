...
export function compileAssemblies(inputs,options={}){
  ...
  for(const a of assemblies){
    ...
    const methods=[];
    for(const m of a.methods){try{const verified=verifyMethod(a,m,options);for(const token of verified.calls){const ref=[...a.methods,...a.members].find(x=>x.token===token);if(!resolveAdapter(ref)&&!assemblies.some(x=>x.name===ref.assembly&&x.methods.some(y=>methodKey(y)===methodKey(ref))))throw new Error(`No linked method or runtime adapter for [${ref.assembly}]${methodKey(ref)}`);}for(const token of verified.fields){const ref=[...a.fields,...a.members].find(x=>x.token===token);if(!assemblies.some(x=>x.name===ref.assembly&&x.fields.some(f=>f.owner===ref.owner&&f.name===ref.name&&typeName(f.type)===typeName(ref.type))))throw new Error(`Unresolved field [${ref.assembly}]${ref.owner}::${ref.name}`);}methods.push({method:m,code:emitMethod(a,m,verified),instructions:verified.instructions,peak:verified.peak});}catch(e){const d=diagnostic(e,a.path,m);if(a.source){const at=m.instructions?.find(i=>i.offset===(Number(e.offset)||0))?.sourceOffset??m.body?.offset??0;Object.assign(d,a.source.location(at));}diagnostics.push(d);}}
