...
export function readPortablePdb(input,{maxBytes=16*1024*1024,maxRows=200000,maxPoints=500000}={}){
  ...
  function guid(index){if(!index)return '00000000-0000-0000-0000-000000000000';const s=streams.get('#GUID');if(!s||index*16>s.size)fail('PDB GUID index out of range');const b=r.sub(s.offset+(index-1)*16,16).slice(16);return [hex(b.slice(0,4).reverse()),hex(b.slice(4,6).reverse()),hex(b.slice(6,8).reverse()),hex(b.slice(8,10)),hex(b.slice(10))].join('-');}
