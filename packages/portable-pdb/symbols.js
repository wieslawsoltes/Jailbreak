...
/** Pairs exact Portable PDB identity, optional checksum, metadata counts and IL/local ranges. */
export async function prepareDebugAssembly(input,options={}){
  ...
  const pdb=readPortablePdb(bytes,{...options,maxBytes:options.maxPdbBytes??16*1024*1024});
  if(!directory.codeView.some(c=>c.id===pdb.id))fail('Portable PDB identity does not match the DLL CodeView record');
  const copy=new Uint8Array(bytes);copy.fill(0,pdb.idOffset,pdb.idOffset+20);let checksumVerified=false;
  for(const record of directory.checksums){const algorithm={SHA256:'SHA-256',SHA384:'SHA-384',SHA512:'SHA-512'}[record.algorithm];if(algorithm){if(await digest(copy,algorithm)!==record.hash)fail('Portable PDB checksum does not match the DLL');checksumVerified=true;}}
  ...
  for(const info of pdb.methods){
    ...
    if(info.localSignature!==(body.localToken&0xffffff)&&info.points.length)fail('PDB local signature does not match method IL');
  ...
  const sources=await pdbSources(pdb,options);
  verified.set(assembly,{id:pdb.id,checksumVerified,documents:pdb.documents.map(({embedded,...d})=>d),methods:pdb.methods,sources,stateMachines:pdb.stateMachines});return assembly;
}
