...
async function main(){
 const argv=process.argv.slice(2),inputs=[];let out='converted-library',targetFramework;
 for(let i=0;i<argv.length;i++){const value=argv[i];if(value==='--out'||value==='--tfm'){if(!argv[i+1])throw new Error('Missing value for '+value);if(value==='--out')out=argv[++i];else targetFramework=argv[++i];}else if(value.startsWith('--'))throw new Error('Unknown option '+value);else inputs.push(value);}
 if(!inputs.length)throw new Error('Usage: node scripts/compile-binary.mjs file.il | library.dll [...] | package.nupkg [...] [--tfm net8.0] [--out path/prefix]');
 if(inputs.length>32)throw new Error('At most 32 input files');
 const files=await Promise.all(inputs.map(async name=>{const stat=await fs.stat(name);if(stat.size>32*1024*1024)throw new Error('Input exceeds 32 MiB: '+name);return {path:name,bytes:new Uint8Array(await fs.readFile(name))};}));
 let result;if(files.length===1&&/\.il$/i.test(inputs[0]))result=compileIL(new TextDecoder().decode(files[0].bytes),{path:inputs[0]});
 else if(inputs.every(n=>/\.(dll|exe)$/i.test(n)))result=compileAssemblies(files);
 else if(inputs.every(n=>/\.nupkg$/i.test(n)))result=await convertNugetPackages(files,{targetFramework});
 else throw new Error('Supply one IL source file, managed assemblies, or NuGet packages; do not mix input formats');
