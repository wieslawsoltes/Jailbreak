import {inlineAssetJson} from './html-assets.mjs';
...
await fs.appendFile(path.join(site,'style.css'),'\n'+await fs.readFile(path.join(root,'apps/ide/studio.css'),'utf8'));
await fs.writeFile(path.join(site,'runtime.js'),await bundleRuntime(root));await fs.writeFile(path.join(site,'.nojekyll'),'');
await fs.appendFile(path.join(site,'style.css'),'\n'+(await Promise.all(['desktop.css','desktop-documents.css','desktop-tools.css','desktop-floating.css','desktop-views.css','desktop-menus.css','desktop-responsive.css'].map(name=>fs.readFile(path.join(root,'apps/ide',name),'utf8')))).join(''));
...
const binaryHtml=await fs.readFile(path.join(site,'binary/index.html'),'utf8'),binaryCss=await fs.readFile(path.join(root,'apps/binary/style.css'),'utf8');
...
await fs.writeFile(path.join(site,'binary/index.html'),binaryHtml.replace('<link rel="stylesheet" href="./style.css">',()=>'<style>'+binaryCss+'</style>').replace('<script type="module" src="./app.js"></script>',()=>'<script>globalThis.__JailbreakBinaryAssets='+inlineAssetJson(binaryAssets)+';</script><script>'+binaryApp.replace(/<\/script/gi,'<\\/script')+'</script>'));
...
// The IDE embeds Binary Studio too; switching tools never needs a CDN or a network.
assets.binaryHtml=await fs.readFile(path.join(site,'binary/index.html'),'utf8');
