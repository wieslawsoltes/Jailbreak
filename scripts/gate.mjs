...
let failed=false;
async function readFiles(dir,base='',files={}){for(const e of await fs.readdir(dir,{withFileTypes:true})){if(['bin','obj','.git','node_modules'].includes(e.name))continue;const name=base+e.name;if(e.isDirectory())await readFiles(path.join(dir,e.name),name+'/',files);else if(/\.(cs|a?xaml|csproj|sln|slnx|props|targets|json|md|txt)$/i.test(name)){if(Object.keys(files).length>3000)throw new Error('Gate source limit exceeded');files[name]=await fs.readFile(path.join(dir,e.name),'utf8');}}return files;}
for(const fixture of [...baseline.official.files,...(baseline.requestedLibrary.files??[])]){const bytes=await fs.readFile(path.join(root,fixture.local));const blob=crypto.createHash('sha1').update(`blob ${bytes.length}\0`).update(bytes).digest('hex');const passed=blob===fixture.blob;report.fixtureIntegrity.push({...fixture,actual:blob,passed});if(!passed)failed=true;}
for(const entry of await fs.readdir(path.join(root,'examples'),{withFileTypes:true})){if(!entry.isDirectory())continue;const files=await readFiles(path.join(root,'examples',entry.name));const compiled=await compileWorkspaceInputs(files),item={name:entry.name,compiled:compiled.success,hydrated:false,browserBehavior:'separate browser gate',statistics:compiled.stats,diagnostics:compiled.diagnostics};
