...
class StudioTests(unittest.TestCase):
    ...
    def test_top_debugger_gutter_locals_edit_and_continue_execute_real_csharp(self):
        ...
        self.page.fill('#studio-local-input','7');self.page.click('#studio-local-apply')
        expect(self.page.locator('#studio-locals')).to_contain_text('7')
        self.page.click('#studio-tab-stack')
    ...
    def test_designer_perspective_properties_and_hot_reload_share_runtime(self):
        ...
        self.page.click('#studio-view-design')
        self.page.frame_locator('#preview').get_by_role('button',name='Increment',exact=True).click()
        expect(self.page.get_by_label('Design Content',exact=True)).to_have_value('Increment')
        self.page.fill('#studio-property-search','Content')
    ...
    def test_embedded_binary_compiler_runs_and_preserves_source_app(self):
        ...
        self.page.click('#studio-view-binary')
        ...
        self.page.click('#studio-view-split')
    ...
    def test_error_list_search_and_navigation_use_real_compiler_diagnostics(self):
        ...
        self.page.locator('#editor').fill(code);self.page.click('#run')
        ...
        self.page.fill('#studio-diagnostic-search','nonexistent-diagnostic')
        self.assertEqual(self.page.locator('.diagnostic-row:visible').count(),0)
        self.page.fill('#studio-diagnostic-search','UnavailableApi')
    ...
    def test_watch_window_inspects_real_paused_frame_and_rejects_calls(self):
        ...
        self.page.frame_locator('#preview').get_by_role('button',name='Increment',exact=True).click()
        expect(self.page.locator('#studio-session-badge')).to_have_text('Paused')
        self.page.click('#studio-tab-watch')
        self.page.fill('#studio-watch-input','this.count, this.constructor()');self.page.click('#studio-evaluate-watches')
    ...
    def test_breakpoint_and_tool_tabs_support_keyboard_activation(self):
        self.page.locator('#studio-tools-all').focus();self.page.keyboard.press('End');self.page.keyboard.press('Enter')
        expect(self.page.locator('#studio-tools-debug')).to_have_attribute('aria-selected','true')
        self.assertTrue(self.page.locator('#dev-hierarchy-panel').is_hidden())
        self.page.locator('#studio-tab-stack').focus();self.page.keyboard.press('ArrowRight');self.page.keyboard.press('Enter')
        expect(self.page.locator('#studio-tab-locals')).to_have_attribute('aria-selected','true')
        self.assertEqual(self.errors,[])

    def test_mobile_and_light_theme_keep_tools_available_without_overflow(self):
        self.page.click('#theme');self.page.set_viewport_size({'width':390,'height':844})
        ...
        self.page.click('#studio-view-design')
        expect(self.page.locator('#development-panel')).to_be_visible()
    ...
    def test_new_file_dialog_builds_solution_tree_and_rejects_unsafe_paths(self):
        ...
        self.page.fill('#studio-new-file-name','../outside.cs');self.page.click('#studio-new-file-create')
        expect(self.page.locator('#studio-new-file-dialog [role=alert]')).to_contain_text('relative')
        self.page.fill('#studio-new-file-name','Views/Inspector.axaml');self.page.click('#studio-new-file-create')
    ...
    def test_font_size_and_studio_commands_use_existing_workbench(self):
        self.page.select_option('#studio-font-size','16')
        self.assertEqual(self.page.locator('#editor').evaluate('e=>getComputedStyle(e).fontSize'),'16px')
        self.page.keyboard.press('Control+Shift+p');self.page.fill('#wb-palette-query','View Watch window');self.page.keyboard.press('Enter')
        expect(self.page.locator('#studio-tab-watch')).to_have_attribute('aria-selected','true')
        with self.page.expect_download() as d:self.page.click('#save-workspace')
        workspace=json.loads(Path(d.value.path()).read_text())
